...
namespace Domore.Conf {
    [TestFixture]
    public class ConfBlockItemTest {
        string Content;
        IConfBlockItem _Subject;
        IConfBlockItem Subject {
            get => _Subject ?? (_Subject = new ConfBlockFactory().CreateConfBlock(Content).Item(0));
            set => _Subject = value;
        }
        ...
        public void ConvertValue_CreatesInstanceOfType() {
            Content = @"value = domore.Conf.ConfBlockItemTest+Cat, Domore.Conf.Test";
